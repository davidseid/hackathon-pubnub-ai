...
async function main() {
    ...
    console.log("IOT Simulator: An AI sensor fleet on PubNub\n");
    ...
    const numUsers = await promptUser("How many devices do you want?\n");
    if (numUsers == "exit") {
        process.exit(0);
    }
    
    const topic = await promptUser("What type of data should we simulate?\n");
    ...
    const nameCompletion = await openai.createChatCompletion({
        model,
        messages: [
            {role: "system", content: `Please give me a comma separated list of ${numUsers} unique device names appropriate for the ${topic}, just the device names as a comma-separated list, nothing else`}
        ],
      });
    ...
    const initialPrompt = await openai.createChatCompletion({
        model,
        messages: [
            {role: "user", content: `Please give me an example json object that an IOT device might emit with sensor data about ${topic}`}
        ],
    });
    ...
    while (true) {
        ...
        const systemPrompt = `
            ...
            Please don't include any placeholders, just give the json objects.
        `;
